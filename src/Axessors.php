...
trait Axessors
{
	public function __call(string $method, array $args)
	{
		...
		elseif ($method == '__axessors_execute_instance')
		{
			...
			{
				throw new ReThrownError("an error occured while evlauating executable string \"$code\": " . $error->getMessage());
			}
	...
	public static function __callStatic(string $method, array $args)
	{
		...
		elseif ($method == '__axessors_execute_static')
		{
			...
			catch (\ParseError $error)
			{
				...
				throw new ReThrownError("an error occured while evlauating executable string in class $class: " . $error->getMessage());
			}
			if (preg_match('{^\$var\s*(\+|-|/|\*|%)?=\s*.+$}', $code))
			{
				return $result;
