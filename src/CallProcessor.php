...
class CallProcessor
{
	...
	private function run(PropertyData $propertyData, array $args)
	{
		...
		if ($prefix == 'get')
		{
			...
			if ($this->processConditions($value, true))
			{
		...
		elseif ($prefix == 'set')
		{
			...
			if ($this->processConditions($args[0]))
			{
				...
				is_null($this->object) ? $propertyData->reflection->setValue($value) : $propertyData->reflection->setValue($this->object, $value);
				$propertyData->reflection->setAccessible(false);
				return;
			}
			else
		...
		else
		{
			...
			foreach ($this->propertyData->getTypeTree() as $type => $subType)
			{
				...
				{
					if (preg_match('{^m_(in|out)_}', $method->name) && $method->isStatic() && $method->isPublic() && !$method->isAbstract())
					{
	...
	private function checkType(array $typeTree, $var): void
	{
		foreach ($typeTree as $type => $subType)
		{
			if (is_int($type))
			{
				if ($var instanceof $subType || ((new \ReflectionClass($subType))->hasMethod('is') && call_user_func([$subType, 'is'], $var)))
				{
					return;
				}
			}
			...
			{
				if (is_iterable($var) && ($var instanceof $type || ((new \ReflectionClass($type))->hasMethod('is') && call_user_func([$type, 'is'], $var))))
				{
	...
	private function executeHandlers($value)
	{
		...
		foreach ($handlers as $handler)
		{
			if (strpos($handler, '`') !== false)
			{
				...
				{
					$value = call_user_func([$this->reflection->name, '__axessors_execute_staic'], $handler, $value, false);
				}
				else
				{
	...
	private function executeCondition($condition, $value): bool
	{
		if (strpos($condition, '`') !== false)
		{
			...
			if (is_null($this->object))
			{
				return call_user_func([$this->backtrace['class'], '__axessors_execute_static'], $condition, $value, true);
			}
			else
			{
	...
	private function isAccessible(string $accessModifier, \ReflectionClass $reflection): bool
	{
		...
		$isThisBranch     = is_subclass_of($this->class, $reflection->name) || is_subclass_of($reflection->name, $this->class);
