...
for ($i = NUM_OF_PREDEFINED_CLASSES; $i < $totalClasses; ++$i)
{
	...
	if (!in_array(Axessors::class, $reflection->getTraitNames()) &&!in_array(Axs::class, $reflection->getTraitNames()))
	{
		continue;
	}
	...
	foreach ($requiredMethods as $accessModifier => $methods)
	{
		...
		{
			if (!isset($implementedMethods[$accessModifier]) || !in_array($method, $implementedMethods[$accessModifier]))
			{
